...
> 📌 **雙檔存放機制**：每日產生兩份 Markdown — **完整詳細日報**（含每篇新聞/論文/專案的 AI 深度解說）存入對應的主題資料夾；**精簡「今日匯報總結」**（AI 產生摘要 + 重點清單）存入 `8.每日新聞總結` 資料夾。每日還會**自動 commit 並推送至 GitHub**。
...
5. **📋 今日匯報總結（精簡版）**：
   - 由 AI 產生約 150-250 字的精簡總結，附重點新聞/論文/專案清單，存放於 `8.每日新聞總結`
